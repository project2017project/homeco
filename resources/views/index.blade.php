...
@section('frontend-content')
    ...
    @if ($intro_content->visibility)
        ...
        @endphp
		<!-- homec Hero -->
		<section id="hero" class="homec-hero homec-hero__v2 p-relative" style="background-image:url({{ asset($home1_intro->bg_image) }});">
			<div class="container">
					...
					<div class="col-12">
						<div class="homec-hero__inner homec-hero__inner--v2">
							...
							<div class="homec-hero__content homec-hero__content--v2">
								<h1 class="homec-hero__title">{{ $home1_intro->title_1 }} <span class="homec-hero__title--focus homec-hero__title--focus--hero">{{ $home1_intro->title_2}}</span> {{ $home1_intro->title_3 }}</h1>
								<ul class="homec-iconic-list homec-iconic-list__v2 homec-iconic-list__white list-none mg-top-30">
									<li><i class="fa-solid fa-check"></i>{{ $home1_intro->list1 }}</li>
									<li><i class="fa-solid fa-check"></i>{{ $home1_intro->list2 }}</li>
									<li><i class="fa-solid fa-check"></i>{{ $home1_intro->list3 }}</li>
								</ul>
								<a href="{{ route('properties') }}" class="homec-btn homec-btn__second"><span>{{__('user.See all Property')}}</span></a>
							</div>
							<!-- End Hero Content -->
								...
								<div class="swiper mySwiper homec-slider-pcard loading">
									<div class="swiper-wrapper">
										...
										<div class="swiper-slide">
											<!-- Single property-->
											<a href="{{ route('property', html_decode($slider_property->slug)) }}" class="homec-property homec-property__card">
												<!-- Property Head-->
												<div class="homec-property__head">
													<div class="homec-overlay"></div>
													<img src="{{ asset($slider_property->thumbnail_image) }}" alt="thumbnail_image">
												</div>
												<!-- Property Body-->
												<div class="homec-property__body">
													<div class="homec-property__body--inside">
														<div class="homec-property__price">{{ $currency_icon }}{{ html_decode(num_format($slider_property->price)) }}
                                                            @if ($slider_property->purpose == 'rent')
                                                            <span>/{{ $slider_property->rent_period }}</span>
                                                            @endif

                                                        </div>
														<h3 class="homec-property__title">{{ html_decode($slider_property->title) }}</h3>
														<div class="homec-property__text">
															<img src="{{ asset('frontend/img/location-icon2.svg') }}" alt="address"><p>{{ html_decode($slider_property->address) }}</p>
														</div>
														<!-- Property List-->
														<ul class="homec-property__list homec-border-top list-none">
															<li><img src="{{ asset('frontend/img/room-icon3.svg') }}" alt="total_bedroom">{{ $slider_property->total_bedroom }} {{__('user.Bed')}}</li>
															<li><img src="{{ asset('frontend/img/bath-icon3.svg') }}" alt="total_bathroom">{{ $slider_property->total_bathroom }} {{__('user.Bath')}}</li>
															<li><img src="{{ asset('frontend/img/size-icon3.svg') }}" alt="total_area">{{ html_decode($slider_property->total_area )}} {{__('user.m2')}}</li>
														</ul>
													</div>
												</div>
											</a>
