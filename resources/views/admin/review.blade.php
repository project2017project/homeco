                        ...
                        <table class="table table-striped" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th width="15%">{{__('admin.User Name')}}</th>
                            ...
                            <tbody>
                                @foreach ($reviews as $index => $review)
                                    <tr>
                                        <td>{{ ++$index }}</td>
                                        <td><a target="_blank" href="{{ route('admin.customer-show', $review->user_id) }}">{{ $review->user ? html_decode($review->user->name) : '' }}</a></td>
                                        <td><a href="{{ route('admin.property.edit', $review->property_id) }}">{{ html_decode($review->property->title) }}</a></td>
